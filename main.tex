...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners=1pt, minimum height=6mm, minimum width=15mm, font=\footnotesize, align=center, inner sep=2pt},
    accent/.style={box, fill=blue!8, draw=blue!60!black},
    >={Stealth[length=1.6mm,width=1.6mm]}
]
\node[box] (env) {Radial feeder \\ + load $x_t$};
\node[accent, right=of env] (lasso) {LASSO \\ support $\hat S_a$};
\node[accent, right=of lasso] (ts) {FG Thompson \\ sampling on $\hat S_a$};
\node[box, right=of ts] (act) {action $a_t$};
\node[box, below=4mm of ts] (buf) {per-arm buffer $\mathcal{D}_a$};

\draw[->] (env) -- node[above, font=\scriptsize]{$x_t$} (lasso);
\draw[->] (lasso) -- node[above, font=\scriptsize]{$\hat S_a$} (ts);
\draw[->] (ts) -- (act);
\draw[->] (act.south) -- ++(0,-3mm) -| node[pos=0.25, above, font=\scriptsize]{$y_t$} (buf.east);
\draw[->] (buf.west) -| ([yshift=-3mm]lasso.south) -- (lasso.south);
\end{tikzpicture}
\caption{FGTS-LASSO architecture. Periodic LASSO selects an active support $\hat S_a$ from the per-arm buffer; feature-Gaussian Thompson Sampling explores within the recovered subspace, decoupling sparse identification from posterior sampling.}
\label{fig:schematic}
\end{figure}
