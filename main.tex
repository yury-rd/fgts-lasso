...
\title{Conformal Sparse Bandits for Online Voltage Regulation in Distribution Grids}
...
\markboth{IEEE Control Systems Letters,~Vol.~X, No.~Y, Month~Year}{Suraveikin \MakeLowercase{\textit{et al.}}: Conformal Sparse Bandits for Online Voltage Regulation}
...
\IEEEPARstart{T}{he} stochastic contextual bandit is a canonical model for sequential decision-making under uncertainty and underpins applications ranging from online recommendation to adaptive clinical trials and data-driven control \cite{li2010contextual,slivkins2019introduction}. At each round, a learner observes a context $x_t \in \mathbb{R}^d$, selects one of $K$ actions, and receives a stochastic reward whose conditional mean depends linearly on the chosen arm's parameter vector. The classical exploration--exploitation trade-off is well understood in low to moderate dimensions: LinUCB \cite{chu2011contextual} and linear Thompson Sampling \cite{agrawal2013thompson} achieve $\tilde{\mathcal{O}}(d\sqrt{T})$ regret, which is minimax-optimal up to logarithmic factors.

In high-dimensional applications, however, $d$ may be of order thousands while only $s \ll d$ coordinates of each parameter vector are non-zero. The classical bounds become vacuous, and any algorithm whose per-round complexity scales with $d$ or $d^2$ is computationally prohibitive. A growing body of work addresses this regime by leveraging sparsity through $\ell_1$-regularised estimators \cite{bastani2020online,kim2019doubly,oh2021sparsity,hao2020high}, thresholded LASSO \cite{ariu2022thresholded}, posterior sampling with sparse priors \cite{chakraborty2023thompson}, sparsity-agnostic concentration \cite{jin2024sparsity}, and nonparametric extensions \cite{valko2013kernels}. Despite this progress, two practical gaps remain. First, several algorithms either assume a known sparsity level or require carefully tuned exploration budgets that depend on unknown problem constants. Second, methods that combine feature selection with optimism or posterior sampling typically maintain inference over the ambient $\mathbb{R}^d$ space, missing the computational gain that sparse estimation could provide.

\begin{figure}[t]
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners=1pt, minimum height=8mm, minimum width=17mm, font=\footnotesize, align=center, inner sep=1.5pt},
    accent/.style={box, fill=blue!8, draw=blue!55!black},
    >={Stealth[length=1.6mm,width=1.6mm]}
]
% --- Mini IEEE-33-style radial feeder schematic on the left ---
\node[draw, rounded corners=1pt, minimum width=20mm, minimum height=14mm, inner sep=0pt] (env) at (0,0) {};
\begin{scope}[shift={(env.center)}, scale=0.11, every node/.style={inner sep=0pt}]
  % slack (square)
  \draw[fill=black] (-9,2) rectangle ++(2,2);
  \node[font=\tiny\bfseries, anchor=east] at (-9.5,3) {};
  % main feeder buses (5)
  \foreach \i in {0,1,2,3,4} {\filldraw[black] (\i*4,3) circle (1.0);}
  \draw[line width=0.5pt] (-7,3) -- (16,3);
  % lateral
  \filldraw[black] (0,-3) circle (1.0);
  \filldraw[black] (0,-9) circle (1.0);
  \draw[line width=0.5pt] (0,3) -- (0,-9);
  % targets (red)
  \draw[red, line width=1.0pt] (8,3) circle (2.0);
  \draw[red, line width=1.0pt] (16,3) circle (2.0);
  \draw[red, line width=1.0pt] (0,-9) circle (2.0);
\end{scope}
\node[font=\scriptsize] at ($(env.south) + (0,-2mm)$) {radial feeder};

% --- Estimator + actuation chain ---
\node[accent, right=8mm of env] (lasso) {LASSO\\ $\hat S_a$};
\node[accent, right=3mm of lasso] (ts) {FG Thompson\\ sampling};
\node[box, right=8mm of ts] (act) {action $a_t$};
\node[box, below=5mm of lasso, minimum width=17mm] (buf) {buffer $\mathcal{D}_a$};

% Dashed group around the decoupled estimator
\node[draw, dashed, rounded corners=2pt, fit=(lasso)(ts), inner sep=2mm,
      label={[font=\scriptsize\itshape, yshift=-0.5mm]above:decoupled sparse estimator}] (group) {};

% Arrows
\draw[->] (env.east) -- node[above, font=\scriptsize]{$x_t$} ([xshift=-0.5mm]lasso.west);
\draw[->] (lasso.east) -- node[above, font=\scriptsize]{$\hat S_a$} (ts.west);
\draw[->] (ts.east) -- (act.west);
\draw[->] (act.south) |- node[pos=0.78, above, font=\scriptsize]{$y_t$} (buf.east);
\draw[->] (buf.north) -- (lasso.south);
\end{tikzpicture}
\caption{FGTS-LASSO architecture. Per-arm LASSO refits identify a sparse support $\hat S_a$ on the path from the slack bus (square) to a target bus (red); feature-Gaussian Thompson Sampling explores within the recovered subspace. The decoupled estimator (dashed box) reduces per-round cost from $\mathcal{O}(d^2)$ to $\mathcal{O}(s^2)$.}
\label{fig:schematic}
\end{figure}

\textit{Motivating example: voltage regulation on a radial feeder.} Consider a distribution feeder with $d$ non-slack buses and $K$ candidate voltage-regulation policies, each targeting a designated bus. By the LinDistFlow approximation \cite{baran1989network}, the voltage deviation at any bus is a linear function of nodal active and reactive injections; for a radial feeder, the corresponding sensitivity vector has support exactly on the electrical path from the slack bus to the target, so each policy's reward parameter is sparse with $s$ equal to the depth of that bus in the feeder topology. Recent power-systems work has exploited this structure for stochastic optimal power flow under uncertainty \cite{lukashevich2023chance,lukashevich2024scenario}, but learning it from sequential voltage measurements remains underexplored. The operator does not know in advance which buses' load fluctuations drive the regulation performance of a given policy and must learn it online from voltage measurements; this is precisely the setting our algorithm addresses.
...
\begin{itemize}
\item[(C1)] We introduce a two-stage estimator that periodically recovers a per-arm support via LASSO and runs feature-Gaussian Thompson Sampling \cite{agrawal2013thompson,abeille2017linear} on the reduced subspace, reducing per-round cost from $\mathcal{O}(d^2)$ to $\mathcal{O}(s^2)$ once the support stabilises.
...
We work under the following assumptions, which are standard in the high-dimensional bandit literature \cite{bastani2020online,oh2021sparsity,hao2020high}.
...
\centering
\resizebox{\columnwidth}{!}{\input{results/tables_synth_regret.tex}}
\end{table}
...
\label{tab:power}
\centering
\resizebox{\columnwidth}{!}{\input{results/tables_power.tex}}
\end{table}
...
\label{tab:highd}
\centering
\resizebox{\columnwidth}{!}{\input{results/tables_highd.tex}}
\end{table}
...
\label{tab:kernel}
\centering
\resizebox{\columnwidth}{!}{\input{results/tables_kernel.tex}}
\end{table}
...
\subsection{Distribution-free conformal coverage}\label{sec:exp_conformal}

Beyond regret, online voltage-regulation deployments require \emph{calibrated uncertainty}: a controller that commits an action at round $t$ should report a prediction interval for the resulting voltage deviation that covers the true value with prescribed probability. We wrap FGTS-LASSO with a per-arm \emph{split-conformal calibration layer} \cite{vovk2005algorithmic}. At each round, a fraction $1{-}p_{\mathrm{cal}}$ of observations is used to update the LASSO posterior as before, while the remaining fraction populates a per-arm rolling calibration buffer of absolute prediction residuals $\{|y_\tau - x_\tau^\top \mu_{a,\hat{S}_a}|\}$. The conformal half-width for the next round is the standard split-conformal quantile of these residuals \cite{vovk2005algorithmic}; the resulting interval is finite-sample valid for exchangeable calibration data and remains marginally valid under mild adaptive corrections \cite{capone2024learning}.

Figure~\ref{fig:conformal} reports rolling empirical coverage at target level $1-\alpha = 0.9$ and the conformal interval width over $30$ seeds. The empirical coverage stabilises within a few hundred rounds and remains within $5\%$ of the nominal level on every benchmark (final coverage: $0.947$ at $d{=}200$, $0.923$ at $d{=}500$, $0.964$ on IEEE 33-bus); the interval width contracts at the rate predicted by the LASSO estimation bound (Lemma~\ref{lem:lasso}). The conformal wrapper therefore equips FGTS-LASSO with distribution-free coverage guarantees \emph{without} altering the regret bound of Theorem~\ref{thm:main}, since calibration points are drawn from the same policy stream and only $p_{\mathrm{cal}} T$ rounds are diverted from posterior updating.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/conformal_coverage.pdf}
\caption{Conformal coverage of FGTS-LASSO at target level $1-\alpha = 0.9$ (dashed). (a) Rolling empirical marginal coverage; the wrapper holds within $5\%$ of the nominal level on every benchmark. (b) Conformal interval width contracts as the LASSO support stabilises.}
\label{fig:conformal}
\end{figure}

\section{Conclusion}\label{sec:conclusion}

We presented FGTS-LASSO, a decoupled algorithm for high-dimensional sparse linear contextual bandits that combines periodic LASSO support recovery with feature-Gaussian Thompson Sampling on the recovered subspace. The algorithm enjoys a high-probability $\tilde{\mathcal{O}}(s\sqrt{T} + Ks\log d)$ regret bound that matches the minimax rate of \cite{hao2020high} up to logarithmic factors and an $\mathcal{O}(s^2)$ post-recovery per-round cost. Empirically, FGTS-LASSO is the lowest-regret algorithm among the four sparse-aware baselines, LinUCB, and two nonlinear baselines tested, at every $d \geq 200$, dominating LinUCB by up to $2.4\times$ and other sparse-aware baselines by $3$--$5\times$ at $d = 500$. In the extreme-high-dimensional regime $T < d^2$ relevant to data-driven control of large-scale systems, sparse-aware methods cluster on regret while their per-round cost grows linearly in $d$, in contrast to LinUCB's quadratic scaling. On a voltage-regulation benchmark on the IEEE 33-bus distribution feeder, FGTS-LASSO recovers the radial-topology-induced support exactly on every seed. A current limitation is the minimum-signal assumption, which excludes parameter coordinates of magnitude below $\beta_{\min}$. Two natural extensions are to relax this assumption via a Bastani--Bayati-style auxiliary regret term, and to enforce voltage-band safety constraints by combining the recovered support with control-barrier-function certificates \cite{ames2017cbf} or distribution-free conformal voltage envelopes \cite{vovk2005algorithmic}. The latter two directions align with active L-CSS research threads on safe online learning for control \cite{capone2024learning}.
