...
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners=1pt, minimum height=8.5mm, minimum width=22mm, font=\footnotesize, align=center, inner sep=2pt},
    accent/.style={box, fill=blue!10, draw=blue!55!black},
    >={Stealth[length=1.6mm,width=1.6mm]}
]
% Top row: 4 equal-width boxes
\node[box] (ctx) {context $x_t$};
\node[accent, right=3.5mm of ctx] (lasso) {LASSO refit\\[-1pt]\scriptsize(every $\tau_0$)};
\node[accent, right=2.5mm of lasso] (ts) {Thompson sample\\[-1pt]\scriptsize on $\hat S_a$ (every $t$)};
\node[box, right=3.5mm of ts] (arm) {arm $a_t$};

% Dashed group around the decoupled estimator
\node[draw, dashed, rounded corners=2pt, fit=(lasso)(ts), inner sep=2mm,
      label={[font=\scriptsize\itshape, yshift=-0.5mm]above:decoupled estimator}] (group) {};

% Buffer below, wide
\node[box, minimum width=42mm, below=6mm of group] (buf) {per-arm buffer $\mathcal{D}_a$};

% Arrows: solid = fast path, dashed = slow path
\draw[->] (ctx.east) -- (lasso.west);
\draw[->, dashed] (lasso.east) -- node[above, font=\scriptsize]{$\hat S_a$} (ts.west);
\draw[->] (ts.east) -- (arm.west);
\draw[->] (arm.south) |- node[pos=0.95, above, font=\scriptsize]{$y_t$} (buf.east);
\draw[->] (buf.west) -| (lasso.south);
\end{tikzpicture}
\caption{FGTS-LASSO architecture. The decoupled estimator (dashed box) runs LASSO refit on the slow timescale $\tau_0 = \mathcal{O}(Ks\log d)$ to recover the active support $\hat S_a$ (dashed arrow), while feature-Gaussian Thompson Sampling on $\hat S_a$ runs every round (solid arrows). The buffer $\mathcal{D}_a$ feeds both paths.}
\label{fig:schematic}
\end{figure}
